...
> **[図の作成指示]** 完成画面のイメージ。動画・コメント一覧・送信フォーム・アイテム一覧の位置が分かるワイヤーフレーム。
...
![h:540 ](img/create-repository.png)
...
> **[図の作成指示]** 三つの領域を色分けしたワイヤーフレーム。具体的な配置は固定せず、複数案を考えられる見せ方。
...
小さい変更なので Manual モードでいきなり実装させてみましょう。
...
一覧の各データにある`itemId`、`itemName`、`iconUrl`を使います。
...
- コメントだけ: `text`を送信
- アイテムだけ: アイテム一覧から選択した`itemId`を送信
- 両方: `text`と選択した`itemId`を送信
